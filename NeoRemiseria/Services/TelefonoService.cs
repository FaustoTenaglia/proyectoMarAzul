...
public class TelefonoService: TableService<Telefono>{
    public TelefonoService(DbremiseriaContext contexto): base(contexto) { }

    public override async Task<List<Telefono>> GetAll(Expression<Func<Telefono, bool>>? predicado = null){
        IQueryable<Telefono> query = _context.Set<Telefono>();
        if (predicado != null){
    ...
    public override async Task<bool> DeleteItem(uint id){
        ...
        var telefono = await _context.FindAsync<Telefono>(id);
        ...
        // Baja lógica
        telefono.Estado = 'B';
        await this.UpdateItem(telefono);
